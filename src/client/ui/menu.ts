...
export function showMainMenu(container: HTMLElement): void {
  ...
  menu.innerHTML = `<h1>Bowdle</h1><p>Fast bows. Wild jungle. One more match.</p><button data-action="play">Play</button><button data-action="practice">Practice</button><button data-action="settings">Settings</button>`;
  container.append(menu);
  const navigate = (scene: string): void => { location.search = `?scene=${scene}`; };
  const play = (): void => {
    if (loadName()) { navigate("online"); return; }
    ...
    card.querySelector("button")!.addEventListener("click", () => { const issue = nameError(input.value); error.textContent = issue; if (!issue) { saveName(input.value.trim()); navigate("online"); } });
