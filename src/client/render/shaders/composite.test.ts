...
describe("composite shader", () => {
  ...
  it("draws the horizon band, birds and weighted silhouettes", () => {
    for (const name of ["uniform float horizon", "uniform float cameraYaw", "uniform float hurt", "uniform float streaks", "uniform float night", "viewScale()", "nearCrown", "bird", "depthEdge"]) expect(compositeFragmentShader).toContain(name);
  });
});
