...
export function volumeSurfaceY(volume: Volume, matchTimeMs: number): number {
  if (!volume.flood) return volume.max[1];
  const phase = ((matchTimeMs % FLOOD_PERIOD_MS) + FLOOD_PERIOD_MS) % FLOOD_PERIOD_MS;
  return volume.max[1] + (phase < FLOOD_MS ? FLOOD_RISE : 0);
}
...
export function isInWater(map: MapData, x: number, y: number, z: number, matchTimeMs: number): boolean {
  for (const volume of map.volumes) if (volume.kind === "water"
    ...
    && y >= volume.min[1] && y < volumeSurfaceY(volume, matchTimeMs)) return true;
  return false;
}
