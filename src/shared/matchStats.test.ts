...
import { createMatchStats, recordDeath, recordKill, recordRobinHood } from "./matchStats.ts";
import { MEDALS, medalsFor } from "./medals.ts";
import { matchReward } from "./progression.ts";

describe("match statistics and medals", () => {
  it("tracks weapons, arrow-only distances, zip kills, clashes and death streak resets", () => {
    const stats = createMatchStats();
    expect(stats).toEqual({ kills: 0, deaths: 0, assists: 0, headshots: 0, longShots: 0, longestShotM: 0, daggerKills: 0, boulderKills: 0, zipKills: 0, robinHoods: 0, streak: 0, bestStreak: 0, won: false });
    ...
    recordDeath(stats); recordRobinHood(stats);
    recordKill(stats, { weapon: "boulder", headshot: true, distance: 200, onZip: false });
    expect(stats).toEqual({ kills: 4, deaths: 1, assists: 0, headshots: 1, longShots: 1, longestShotM: 35, daggerKills: 1, boulderKills: 1, zipKills: 1, robinHoods: 1, streak: 1, bestStreak: 3, won: false });
    expect(createMatchStats().kills).toBe(0);
  });
  it("awards every medal in table order and excludes On a Roll from Unstoppable", () => {
    const stats = { ...createMatchStats(), kills: 6, bestStreak: 6, headshots: 3, longestShotM: 45, robinHoods: 1, daggerKills: 1, boulderKills: 1, zipKills: 1, assists: 4, won: true };
  ...
  it("XP breakdown includes recorded headshots, long shots and only four paid medals", () => {
    ...
    expect(reward).toEqual({ xp: 750, ink: 23, breakdown: [
      ...
      { label: "Headshots", xp: 75, ink: 0 }, { label: "Long shots", xp: 75, ink: 0 }, { label: "Win", xp: 200, ink: 10 }, { label: "Medals", xp: 100, ink: 0 },
    ] });
