...
for (const map of matchMaps.filter((entry) => entry.creatureSpawns && (!onlyMode || onlyMode === "expedition"))) for (let seed = 1; seed <= seeds; seed += 1) {
  ...
  for (; run.cleared < EXPEDITION_WAVES && room.state.phase === "live" && run.phase !== "over"; tick += 1) {
    ...
    for (const player of room.state.players.values()) if (player.downed) { player.downed = false; player.hp = MAX_HP; }
  ...
  const ok = run.cleared >= EXPEDITION_WAVES && slowWave === 0 && stuck.size === 0;
  if (!ok) failures += 1;
  console.log(`${ok ? "ok  " : "FAIL"} expedition ${map.id.padEnd(11)} seed ${String(seed * 101).padStart(3)}  waves ${String(run.cleared).padStart(2)}  bosses ${run.bosses}  ${(tick / TICK_HZ / 60).toFixed(1)} min  stuck creatures ${stuck.size}  respawned ${room.expedition?.unstuck ?? 0}${stuck.size ? ` (${[...stuck].slice(0, 4).join(" ")})` : ""}  (${((performance.now() - started) / 1000).toFixed(1)} s wall)`);
  await client.leave().catch(() => undefined);
  room.disconnect();
}
