...
export type HudActions = {
  ...
  mutePings?: (playerId: string) => void;
  reportPlayer?: (playerId: string, reason: "offensiveName" | "cheating" | "afk") => void;
};

export type PlayOfHighlight = {
  killerId: string;
  victimId: string;
  distance: number;
  streak: number;
  kind: "longShot" | "streak";
};
...
export class MatchHud {
  ...
  private readonly scoreboardMenu: HTMLDivElement;
  private scoreboardSelfId = "";
  ...
  constructor(container: HTMLElement, onVote: (mapId: string) => void, actions: HudActions = {}) {
    ...
    this.scoreboardMenu = document.createElement("div");
    this.scoreboardMenu.dataset.testid = "scoreboard-menu";
    this.scoreboardMenu.hidden = true;
    this.scoreboardMenu.style.cssText = "position:fixed;z-index:40;display:flex;flex-direction:column;gap:4px;padding:8px;background:#efe3c6;border:3px solid #4a3527;font:18px Gochi Hand;pointer-events:auto";
    this.root.append(this.scoreboardMenu);
    this.scoreboard.addEventListener("contextmenu", (event) => this.onScoreboardMenu(event));
    window.addEventListener("click", () => { this.scoreboardMenu.hidden = true; });
  ...
  kill(message: KillMessage, names: ReadonlyMap<string, string>): void {
  ...
  end(message: MatchEndMessage, names: ReadonlyMap<string, string>, stats: EndStats, maps: readonly MapData[], run?: RunSummary, playOf?: PlayOfHighlight): void {
    ...
    if (playOf) {
      const potm = document.createElement("div");
      potm.dataset.testid = "play-of-the-match";
      potm.style.cssText = "margin:8px 0 14px;padding:10px;border:3px solid #d2531f;background:#fff8e7;font:24px Permanent Marker";
      const killer = names.get(playOf.killerId) ?? playOf.killerId;
      const victim = names.get(playOf.victimId) ?? playOf.victimId;
  ...
  private onScoreboardMenu(event: MouseEvent): void {
    const target = (event.target as HTMLElement | null)?.closest("[data-player-id]") as HTMLElement | null;
    const playerId = target?.dataset.playerId;
    if (!playerId || playerId === this.scoreboardSelfId) return;
    event.preventDefault();
    this.scoreboardMenu.replaceChildren();
    const mute = document.createElement("button"); mute.type = "button"; mute.dataset.testid = "mute-pings"; mute.textContent = "Mute pings";
    mute.addEventListener("click", () => { this.actions.mutePings?.(playerId); this.scoreboardMenu.hidden = true; });
    const report = document.createElement("button"); report.type = "button"; report.dataset.testid = "report-name"; report.textContent = "Report name";
    report.addEventListener("click", () => { this.actions.reportPlayer?.(playerId, "offensiveName"); this.scoreboardMenu.hidden = true; });
    this.scoreboardMenu.append(mute, report);
    this.scoreboardMenu.style.left = `${event.clientX}px`;
    this.scoreboardMenu.style.top = `${event.clientY}px`;
    this.scoreboardMenu.hidden = false;
  }
}
